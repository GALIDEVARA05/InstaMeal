...
export default function ManagerPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-orange-400 via-pink-500 to-rose-500 p-6 relative">
      ...
      <button
            onClick={() => {
              ...
              // ✅ Add this line for popup toast
              toast.success("✅ Logged out successfully!", { theme: "colored" });

              // small delay so user sees the toast before redirect
              setTimeout(() => nav("/login"), 1000);
            }}
            className="absolute top-6 right-8 z-20 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl transition shadow-lg font-bold border-2 border-white"
          >
            Logout
        </button>

      <div className="max-w-5xl w-full mx-auto bg-white shadow-xl rounded-2xl p-6">
        <div className="mb-6 flex justify-center">
          <h2 className="text-3xl font-extrabold text-pink-600 flex items-center gap-2 text-center">
        ...
        <section className="mb-8">
          <h3 className="text-xl font-semibold mb-3">Pending Recharges</h3>
          {pending.length === 0 ? (
            <p className="text-gray-600 text-center font-semibold">No pending recharge requests ✅</p>
          ) : (
            <ul className="space-y-4">
              {pending.map((r) => (
                <li key={r._id} className="flex justify-between items-center p-4 border border-gray-200 rounded-lg hover:shadow-md transition">
                  <div>
                  ...
                  <div className="space-x-2">
                    <button
                      ...
                      className="px-4 py-2 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-lg hover:scale-105 transform transition"
                    >
                      Approve
                    </button>
                    <button
                      onClick={() => process(r._id, "reject")}
                      className="px-4 py-2 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-lg hover:scale-105 transform transition"
                    >
                      Reject
                    </button>
                  </div>
        ...
        <section className="mb-8 border-t pt-6">
          <h3 className="text-xl font-semibold mb-3">Create Card (by Roll No)</h3>
          <div className="flex gap-3">
            <input
              value={rollNo}
              onChange={(e) => setRollNo(e.target.value)}
              className="border border-gray-300 rounded-xl px-3 py-3 focus:outline-none focus:ring-2 focus:ring-pink-500 flex-1"
              placeholder="Enter Roll No (e.g., 21K61A0501)"
            />
            <button
              onClick={createCard}
              className="px-5 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:scale-105 transform transition"
            >
              Create Card
            </button>
          </div>
        ...
        <section className="border-t pt-6">
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-5">
            <input
              ...
              className="border border-gray-300 rounded-xl px-3 py-3 focus:outline-none focus:ring-2 focus:ring-rose-500 flex-1"
            ...
            <input
              ...
              className="border border-gray-300 rounded-xl px-3 py-3 focus:outline-none focus:ring-2 focus:ring-rose-500 w-36"
              placeholder="Price"
            />
            <select
              value={mCategory}
              onChange={e => setMCategory(e.target.value)}
              className="border border-gray-300 rounded-xl px-3 py-3 focus:outline-none focus:ring-2 focus:ring-rose-500 w-44"
            >
            ...
            <button
              onClick={createMeal}
              className="px-5 py-3 rounded-xl bg-gradient-to-r from-rose-500 to-red-500 text-white hover:scale-105 transform transition"
            >
              Add Meal
            </button>
          </div>

          {/* ✅ NEW: Filters (same categories as StudentPage, plus Dinner) */}
          ...
          {/* Meals list (now uses filteredMeals) */}
                  ...
                  <div className="relative w-full h-36">
                    <div className="w-full h-36 flex items-center justify-center bg-white">
                      <img
                        ...
                        alt={m.name}
                        className={`max-h-32 max-w-full object-contain border ${m.imageUrl && m.imageUrl.trim() !== '' ? 'border-gray-200' : 'border-amber-400'}`}
                    ...
                    {!m.available && (
                      <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-10">
                        ...
                        <span className="text-xs text-gray-500">(Hidden from students)</span>
                      </div>
                    )}
                  </div>
                  <div className="p-4">
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      ...
                      <button
                        ...
                        className="w-full py-2 rounded-lg bg-blue-500 hover:bg-blue-600 text-white font-semibold transition text-sm"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => deleteMeal(m._id)}
                        className="w-full py-2 rounded-lg bg-rose-500 hover:bg-rose-600 text-white font-semibold transition text-sm"
          ...
          {editMeal && (
            <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
              <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-md relative">
                ...
                <h3 className="text-xl font-bold mb-4 text-blue-600">Edit Meal</h3>
                <div className="flex flex-col gap-3">
                  <input
                    value={editName}
                    onChange={e => setEditName(e.target.value)}
                    className="border border-gray-300 rounded-xl px-3 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Meal name"
                  />
                  <input
                    type="number"
                  ...
                  <button
                    ...
                    className="px-5 py-3 rounded-xl bg-blue-500 hover:bg-blue-600 text-white font-semibold mt-2"
                  >
                    Save Changes
                  </button>
                </div>
