...
export default function StudentPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // pick the first card as selected by default (if exists)
        const currentCard = cardsData.length > 0 ? cardsData[0] : null;
        if (currentCard) setSelected(currentCard);

        // merge meals with selectedItems from DB
        const mealsWithSelected = (mealsRes.data || []).map((m) => {
          const selectedItem = currentCard?.selectedItems?.find(
            (it) => String(it.meal?._id || it.meal) === String(m._id)
          );

          // ✅ safe default price
          const basePrice =
            Array.isArray(m.priceOptions) && m.priceOptions.length > 0
              ? Number(m.priceOptions[0])
              : Number(m.price) || 0;
  ...
  // ✅ Add to card
  async function addItem(mealId, qty = 1, price) {
    ...
    setSelected((prev) => {
      ...
      const idx = items.findIndex(
        (it) => it.meal?._id === mealId && it.price === price
      );
      if (idx > -1) {
        items[idx] = { ...items[idx], quantity: (items[idx].quantity || 0) + qty };
      } else {
        const meal = meals.find((m) => m._id === mealId);
        items.push({ meal, quantity: qty, price });
      }
      return { ...prev, selectedItems: items };
    });

    try {
      await api.post("/cards/add-item", {
        cardId: selected._id,
        mealId,
        price: Number(price),
        quantity: qty,
  ...
  async function decrementItem(mealId, price) {
    if (!selected) return;
    ...
    try {
      await api.post("/cards/decrement-item", {
        cardId: selected._id,
        mealId,
        price: Number(price),
      });
  ...
  // ✅ Remove all of that meal+price
  async function removeAllItem(mealId, price) {
    if (!selected) return;
    ...
    try {
      await api.post("/cards/remove-item", {
        cardId: selected._id,
        mealId,
        price: Number(price),
      });
  ...
  async function mockRecharge() {
    if (!selected)
      return toast.error("⚠️ Select a card first", { theme: "colored" });
                  ...
                  {selectedItems.map((it, idx) => (
                    ...
                    >
                      <span className="text-gray-800 truncate">
                        {it.meal?.name} ({it.price}) × {it.quantity}
                      </span>
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => decrementItem(it.meal?._id, it.price)}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {filteredMeals.map((m) => {
                    const item = selectedItems.find(
                      (it) =>
                        it.meal?._id === m._id && it.price === m.selectedPrice
                    );
                    return (
                      <div
                        key={m._id}
                        ...
                        <div className="p-2 sm:p-3 flex flex-col justify-between h-1/2 md:h-auto">
                          ...
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <select
                              ...
                              value={Number.isFinite(m.selectedPrice) ? m.selectedPrice : 0}
                              onChange={(e) => {
                                const newPrice = Number(e.target.value);
                                setMeals((prev) =>
                                  prev.map((meal) =>
                                    meal._id === m._id
                                      ? { ...meal, selectedPrice: newPrice }
                                      : meal
                                  )
                                );
                              }}
                          ...
                          <div className="mt-auto flex justify-center items-center gap-1 sm:gap-2">
                            <button
                              ...
                              onClick={() =>
                                decrementItem(m._id, m.selectedPrice)
                              }
                            ...
                            <button
                              ...
                              onClick={() => addItem(m._id, 1, m.selectedPrice)}
                              className={`px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-lg text-white text-xs sm:text-sm transition ${
                                !m.available || loading
                                  ? "bg-gray-300 cursor-not-allowed"
